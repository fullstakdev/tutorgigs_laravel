<?php
require_once $_SERVER['DOCUMENT_ROOT'].'/MeritHub/MeritHub.Keys.php';

  $arrTypeTutoring=array('intervene'=>'Intervene','drhomework'=>'Drhomework' );
  $arrBoardResource=array('groupworld','newrow','MeritHub');
                  ...
                  <table class="table-manager-user col-md-12">
                     ...
                     <tr>
                        <td> 
                           ...
                           <?php } 
                           elseif($row['board_type']=='MeritHub'){

                              $hostLink=mysql_fetch_assoc(mysql_query("SELECT hostLink FROM `MeritHubClass`WHERE  `sessionID`=".$row['id']));
                              $url=$hostLink['hostLink'];
                              $urlHit="https://live.merithub.com/info/device-test/$NetworkId/$url";  ?>
                           <a title="Join as observer MeritHub" target="_blank" 
                              href="<?=$urlHit?>" 
                              class="btn btn-danger btn-xs">Join as observer MeritHub</a>
                           <?php } else { ?>
                           ...
                           if($row['board_type']=='MeritHub'){?>
                            <br/> <br/>
                           <?php $mod_url="moderator_join_roomMerit.php?sesid=".$row['id'];?>
                        ...
                        <td style="text-align:center">
                           ...
                           <br><br>
                           <strong class="text-primary">Tutor<br></strong> 
                           <?=$Tutor['f_name']." ".$Tutor['lname']?><br/>
                           Email: <?=$Tutor['email'];?><br/>
                           <?php }
                              if($row['add_observer'] == 1 && $row['tut_observer_id'] > 0){
                                      $tut_observer = mysql_fetch_assoc(mysql_query(" SELECT * FROM gig_teachers WHERE 1 AND id=".$row['tut_observer_id']));
                                      ?>
                           <hr>
                           <strong class="text-primary">Observer:</strong> 
                           <?=$tut_observer['f_name']." ".$tut_observer['lname']?>(ME)<br/>
                           Email: <?=$tut_observer['email'];?><br/>
                           <?php }
                              }else{
                                 ?>
                           ...
                           <?php 
                              if($row['add_observer'])
                              {
                                 echo '<span class="btn btn-info btn-xs" style="margin-top:8px;margin-bottom:8px">Observer</span>';
                              }
                              else 
                                  echo '<span class="btn btn-primary btn-xs" style="margin-top:8px;margin-bottom:8px">Tutor</span>';   
                              ?>
                        </td>
                        <td>
                           <span class="btn btn-primary btn-xs">Virtual board:
